...
from mbed_tools.devices._internal.file_parser import extract_product_code_from_htm, extract_online_id_from_htm
...
def resolve_board(candidate: CandidateDevice) -> Board:
    ...
    """
    product_code = extract_product_code_from_htm(candidate.mount_points)
    if product_code:
        try:
    ...
    online_id = extract_online_id_from_htm(candidate.mount_points)
    if online_id:
        slug = online_id.slug
